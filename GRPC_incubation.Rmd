...
freqData <- plyr::ddply( .data = fullBouts, 
                         ...
                         .fun = function( .df ){
                           ...
                           .start <- min( .df$EventStart )               # record starts on date of first iButton off-bout record
                           .end <- max( .df$EventStart )                 # and ends on the date after the last record
                           hour( .start ) <- 1
                           hour( .end ) <- 22
                           minute( .start ) <- minute( .end ) <- 30      # always going from 01:30:00 am
                           second( .start ) <- second( .end ) <- 0       # until 01:29:59 am
                           ...
                           data.frame( blockStart = .starts, 
                                       ...
                                       VPD = .vpd ) } ) %>% 
  mutate( fYear = as.factor( year( blockStart ) ), 
          Jday = yday( blockStart ), 
          Date = ymd( date( blockStart ), tz = "America/Chicago" ) ) %>% 
  left_join( y = dplyr::select( fullBouts, 
                                id, NestAge, VOR_AVG, 
                                contains( "Distance", ignore.case = TRUE ), 
                                contains( "Date", ignore.case = TRUE ), Nest_Success ), 
             by = c( "id", "Date" ) )
...
mcp <- SpatialPolygons( list( Polygons( list( Polygon( nestLocs[ chull( nestLocs ), ] ) ), 
                                        ID = "nestMCP" ) ), 
                        proj4string = CRS( targProj ) ) # coordinates for the minimum convex polygon subtending the 30 nests
...
globalBoutsMod <- "numBouts ~ fYear + NestAge + VOR_AVG + WindSpeed * Turbine_Distance_m + Roads_Distance_m + Water_Distance_m * Jday"
