...
We placed Thermochron$^{\circledR}$ iButton$^{\circledR}$ temperature loggers (model DS1921G; Maxim Integrated, San Jose, CA, USA; hereafter iButtons) in prairie-chicken nests to record nest temperature at 4-minute intervals. The iButtons were placed variously before or after clutch completion (and initiation of incubation) depending upon when the nest was located, and were retrieved after the eggs hatched or the nest failed (or was abandoned). Consequently, not all nests were monitored throughout the entire 28-day incubation period.

**<< Methods on spatial data? >>**

####**Data Analyses** {#datamethods}

We **<< manually? >>** identified off-bouts and measured their duration from iButton temperature data in Raven Pro (Version 1.5; Cornell Lab of Ornithology, Ithaca, NY, USA), using Rhythm [@cooper2005] to convert the files into an appropriate format. We defined off-bouts from the time of the temperature recording just prior to the bird's departure from the nest (indicated by a sudden decrease in temperature in the time-series) until the time of the recording made just prior to her return (indicated by a sharp rise in temperature). Because the precise signature of off-bouts varied, we based our classification on the following criteria: (1) a minimum duration of 10 minutes, (2) a total temperature difference of at least 2°C, and (3) a minimum initial absolute slope in the temperature--time plot of 0.1°C/minute (0.4°C between sequential recordings). **<< These criteria are from Cole's poster (w/ #1 modified based on discussion w/ EJ); should I instead just cite the poster? >>**
...
We created (generalized) linear mixed models ([G]LMMs) of off-bout duration and frequency using nest (hen) ID as a grouping factor in the random-effects structure. A Box-Cox power transformation of off-bout duration suggested that the variable was approximately log-normally distributed (maximum likelihood estimate for $\lambda$ = `r l_est.dur`). We therefore used a Gaussian error distribution and identity link function to model the natural-log-transformed duration.

Using a global (maximal) fixed-effects structure, we fit models with different random-effects strctures and selected among them using Akaike's Information Criterion with a second-order correction for small sample size (AIC$_c$; [@anderson2008]) following the procedure described by Zuur et al. [-@zuur2009] for selecting the most parsimonious mixed-effects model. The LMMs of off-bout duration were fit using restricted maximum likelihood (REML) to minimize bias in the random-effects estimates [@zuur2009]. We then selected a fixed-effects structure using the top-ranked random effects structure. Whereas Zuur et al. [-@zuur2009] recommend selecting the optimal fixed-effects structure by stepwise backwards selection from the global model, we compared this approach with one using AIC$_c$-based model selection. In both cases, (and in both the duration and frequency analyses) models having different fixed effects were fitted and compared using maximum likelihood (ML) methods.

We performed all calculations and statistical procedures in R (version 3.X.X; R Foundation for Statistical Computing, Vienna, Austria) using the package lme4 [@R-lme4] for functionality associated with mixed-effects modeling.
...
CC <- convCheckR( freq.ranef_cs ) # 26 warnings and at least as many minutes.
# UnID        : NONE!
# LargeMaxGrad: all year, all temp, all vor, wind.int, turb.int, all blck
# LargeParGrad: all year, all temp, all vor, all blck
# Singular    : a slew (but not all)

# Okay. Try the optimizer used on the duration models (Nelder-Mead).
freq.ranef_cs.plus.neldermead <- lapply( X = freq.ranef_forms, 
                                         FUN = function( .ff ) glmer( .ff, 
                                                                      data = csFreqData, 
                                                                      family = binomial( link = "logit" ) ) )

freq.ranef_fit <- freq.ranef_cs

```

```{r FREQranefSelection}

op <- AICcmodavg::c_hat( freq.ranef_fit[[ length( freq.ranef_fit ) ]] )
...
freq.ranef_topMod <- freq.ranef_fit[[ "int" ]]

```

We modeled off-bout frequency as the log-likelihood that a hen initiated an off-bout during a particular 3-hour block of time during the recording period (*i.e.*, from the time of the first recorded off-bout until the nest was presumed to have hatched or failed). The latter model assumed a Bernoulli error distribution and logit link function.

```{r FREQfixefSelection}

# ## ITERATION 1: ##
# 
# freq.fixef <- glmer( numBouts ~ fYear + NestAge * AmbientTemp + Precipitation + VOR_AVG + WindSpeed * Turbine_Distance_m + Roads_Distance_m + Water_Distance_m * Jday + ( 1 | fID ), 
#                      family = binomial( link = "logit" ), 
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: VOR_AVG (p=0.985825)

# ## ITERATION 2: ##
# 
# freq.fixef <- glmer( numBouts ~ fYear + NestAge * AmbientTemp + Precipitation + WindSpeed * Turbine_Distance_m + Roads_Distance_m + Water_Distance_m * Jday + ( 1 | fID ),
#                      family = binomial( link = "logit" ),
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: Roads_Distance_m (p=0.711681)

# ## ITERATION 3: ##
# 
# freq.fixef <- glmer( numBouts ~ fYear + NestAge * AmbientTemp + Precipitation + WindSpeed * Turbine_Distance_m + Water_Distance_m * Jday + ( 1 | fID ),
#                      family = binomial( link = "logit" ),
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: Water_Distance_m:Jday (p=0.621493)
...
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: WindSpeed:Turbine_Distance_m (p=0.537227)
...
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: NestAge:AmbientTemp (p=0.512824)
...
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: Water_Distance_m (p=0.368459)

# ## ITERATION 7: ##
# 
# freq.fixef <- glmer( numBouts ~ fYear + NestAge + AmbientTemp + Precipitation + WindSpeed + Turbine_Distance_m + Jday + ( 1 | fID ),
#                      family = binomial( link = "logit" ),
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: AmbientTemp (p=0.294402)

# ## ITERATION 8: ##
# 
# freq.fixef <- glmer( numBouts ~ fYear + NestAge + Precipitation + WindSpeed + Turbine_Distance_m + Jday + ( 1 | fID ),
#                      family = binomial( link = "logit" ),
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: fYear (p=0.167429)

# ## ITERATION 9: ##
# 
# freq.fixef <- glmer( numBouts ~ NestAge + Precipitation + WindSpeed + Turbine_Distance_m + Jday + ( 1 | fID ),
#                      family = binomial( link = "logit" ),
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: Turbine_Distance_m (p=0.127381)

# ## ITERATION 10: ##
# 
# freq.fixef <- glmer( numBouts ~ NestAge + Precipitation + WindSpeed + Jday + ( 1 | fID ),
#                      family = binomial( link = "logit" ),
#                      data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: Precipitation (p=0.062821)

# ## ITERATION 11: ##
# 
freq.fixef <- glmer( numBouts ~ NestAge + WindSpeed + Jday + ( 1 | fID ),
                     family = binomial( link = "logit" ),
                     data = csFreqData )
# 
# summary( freq.fixef ) # least significant term: NestAge (p=0.029678)

freq.mod_be <- freq.fixef

```

```{r FREQvalidatR}

freq.test <- augment( freq.mod_be ) %>% 
  mutate( NestAge = ( NestAge * sd( freqData$NestAge, na.rm = TRUE ) ) + mean( freqData$NestAge, na.rm = TRUE ), 
          ...
          .std.resid = std.resid.glmer( freq.mod_be ), 
          .qresid = qres2( freq.mod_be ), 
          .hat = hatvalues( freq.mod_be ) ) %>% 
  dplyr::select( -.rownames )
# ests <- influence.ME::influence( freq.mod_be, obs = TRUE )
# dur.test$.cooksd <- cooks.distance( ests )[ which( !is.na( freqData$WindSpeed ) & !is.na( freqData$NestAge ) ) ]

rVf_freq <- ggplot( data = freq.test, 
                    aes( x = .fitted, 
                         y = .qresid ) ) + 
  geom_smooth( colour = "seashell4" ) + 
  geom_hline( yintercept = 0, 
...
y <- quantile( x = freq.test$.qresid, probs = c( 0.25, 0.75 ) )
x <- qnorm( p = c( 0.25, 0.75 ) )
slope <- diff( y ) / diff( x )
int <- y[ 1L ] - ( slope * x[ 1L ] )

qq_freq <- ggplot( data = freq.test, 
                   aes( sample = .qresid ) ) + 
  geom_abline( slope = slope, 
               intercept = int, 
               colour = "tomato" ) + 
...
lev_freq <- ggplot( data = freq.test, 
                    aes( x = .hat, 
                         y = .qresid ) ) + 
  geom_hline( yintercept = 0,
              size = 1,
...
cowplot::plot_grid( rVf_freq, qq_freq, sl_freq, lev_freq, nrow = 2, labels = "AUTO" )

```

```{r FREQcheckR}

rVna <- ggplot( data = freq.test, 
                aes( x = NestAge, 
                     y = .qresid ) ) + 
  geom_point( alpha = 0.4 ) + 
  theme_classic() + 
...
cowplot::plot_grid( rVna, rVws, rVjd, fVo, nrow = 2, labels = "AUTO" )
...
nd <- expand.grid( NestAge = 0, 
                   WindSpeed = 0, 
                   Jday = ( seq( from = floor( min( freqData$Jday, na.rm = TRUE ) ), 
                                 to = ceiling( max( freqData$Jday, na.rm = TRUE ) ), 
                                 by = 0.5 ) - mean( freqData$Jday, na.rm = TRUE ) ) / sd( freqData$Jday, na.rm = TRUE ) )
...
freq.predict_jd.plot <- ggplot( data = freq.predict_jd, 
                                aes( x = Jday_r, 
                                     y = .mu ) ) + 
  geom_line() + 
  geom_rug( data = filter( freqData, numBouts > 0 ), 
