...
Our study was part of a broader investigation of the ecological impacts of an existing wind energy facility on prairie-chickens. During the 2013 and 2014 breeding seasons, we monitored prairie-chicken nests in a `r prettyAreaKm2`-km$^2$ area centered at `r round(abs(centroid[1]),2)`°W, `r round(abs(centroid[2]),2)`°N, in Brown county, Nebraska, USA (Figure \@ref(fig:BrownCo)). The area is part of the Nebraska Sandhills, a **AREA**-km$^2$ region characterized by dunes of sandy soil stabilized by mixed-grass prairie (**CITATION**). Between 1981 and 2010, the area near the study site received an average of `r round(totPrecip[1],2)` in (`r round(totPrecip[2])` mm) of precipitation annually, of which `r round(breedingPrecip.percent,1)`% fell during the breeding season (May--July; @hprcc2017). Temperatures ranged from an average monthly low of `r round(minTemp$Fahr,1)`°F (`r round(minTemp$Cels,1)`°C) in `r minTemp$Month` to an average monthly high of `r round(maxTemp$Fahr,1)`°F (`r round(maxTemp$Cels,1)`°C) in `r maxTemp$Month` [@hprcc2017]. Average monthly temperatures during the breeding season were `r round(mayTemp$Fahr,1)`°F (`r round(mayTemp$Cels,1)`°C) in May, `r round(juneTemp$Fahr,1)`°F (`r round(juneTemp$Cels,1)`°C) in June, and `r round(julyTemp$Fahr,1)`°F (`r round(julyTemp$Cels,1)`°C) in July.

The study site encompassed a wind energy facility owned and managed by the Nebraska Public Power District [@nppd2017]. The facility is centered at `r round(abs(centroid.t[1]),2)`°W, `r round(abs(centroid.t[2]),2)`°N and covers a total area of approximately `r round(area.Km2.t,1)` km$^2$. 
...
dur.fixef_aicc <- sapply( X = dur.fixef_fits, FUN = AICc )
...
dur.fixef_FEs <- sapply( X = dur.fixef_fits, FUN = function( .mm ) fetch.FE( .mm ) )
dur.fixef_nFE <- sapply( X = dur.fixef_fits, FUN = function( .mm ) length( getME( .mm, "fixef" ) ) )
dur.fixef_nRE <- sapply( X = dur.fixef_fits, FUN = function( .mm ) length( getME( .mm, "theta" ) ) )
...
dur.predict_aicc.td <- data.frame( TempDiff = ( seq( from = floor( min( fullBouts$TempDiff, na.rm = TRUE ) ), 
                                                     to = ceiling( max( fullBouts$TempDiff, na.rm = TRUE ) ), 
                                                     by = 0.5 ) - mean( fullBouts$TempDiff, na.rm = TRUE ) ) / sd( fullBouts$TempDiff, na.rm = TRUE ), 
                                   WindSpeed = 0 )

dur.predict_aicc.boot <- bootMer( x = dur.mod_aicc, 
                                  FUN = boot.fitter, 
                                  nsim = 1000 )

dur.predict_aicc.bootCI <- t( apply( X = dur.predict_aicc.boot$t, 
                                     MARGIN = 2, 
                                     FUN = quantile, 
                                     probs = c( 0.025, 0.975 ) ) )
...
dur.predict_aicc.ws <- data.frame( TempDiff = 0, 
...
dur.predict_aicc.ws <- dur.predict_aicc.ws %>%
  mutate( .fitted = predict( dur.mod_aicc, 
                             newdata = dur.predict_aicc.ws, 
                             re.form = ~0, 
                             type = "response" ), 
          lo.95 = dur.predict_bootCI[ , 1 ],
          hi.95 = dur.predict_bootCI[ , 2 ],
...
globalBoutsMod <- "numBouts ~ fYear + NestAge * AmbientTemp + Precipitation + VOR_AVG + WindSpeed * Turbine_Distance_m + Roads_Distance_m + Water_Distance_m * Jday"
...
CC <- convCheckR( freq.ranef_cs ) # 0 warnings!?
# UnID        : NONE!
# LargeMaxGrad: NONE!
# LargeParGrad: NONE!
# Singular    : all. (is that a binomial thing, perhaps?)
