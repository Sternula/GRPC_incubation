...
nd <- data.frame( TempDiff = 0, 
                  WindSpeed = 0, 
                  NestAge = 0, 
                  Turbine_Distance_m = ( seq( from = floor( min( fullBouts$Turbine_Distance_m, na.rm = TRUE ) ), 
                                              to = ceiling( max( fullBouts$Turbine_Distance_m, na.rm = TRUE ) ), 
                                              by = 100 ) - mean( fullBouts$Turbine_Distance_m, na.rm = TRUE ) ) / sd( fullBouts$Turbine_Distance_m, na.rm = TRUE ) )

dur.predict_aicc2.tbd <- nd %>% 
  mutate( .fitted = predict( dur.mod_aicc2.p, 
                             newdata = nd, 
                             re.form = ~0, 
                             type = "response" ), 
          Turbine_Distance_m_r = ( Turbine_Distance_m * sd( fullBouts$Turbine_Distance_m, na.rm = TRUE ) ) + mean( fullBouts$Turbine_Distance_m, na.rm = TRUE ), 
          .mu = exp( .fitted ) )
...
fff <- freq.predict.age %>% 
  mutate( NestAge_r = round( NestAge_r ) ) %>% 
  dplyr::select( blockID, .mu, NestAge_r, Jday_r ) %>% 
  filter( NestAge_r %in% c( 0, 14, 28 ) )
...
freq.predict.time_plot <- ggplot( data = filter( freq.predict.age, Jday_r %in% c( 124, 180 ), NestAge_r == 14 ), 
                                  aes( x = blockID, 
                                       y = .mu, 
                                       shape = factor( Jday_r ) ) ) + 
  ...
  scale_shape( labels = c( "124" = "4 May", 
                           "180" = "29 June" ) )
...
freq.predict.age_plot <- ggplot( data = filter( freq.predict.age, Jday_r == 152, round( NestAge_r ) %in% c( 0, 28 ) ), 
                                 aes( x = blockID, 
                                      y = .mu, 
                                      shape = factor( round( NestAge_r ) ) ) ) + 
  geom_point( position = position_dodge( width = 0.5 ) ) + 
  geom_linerange( aes( ymin = lo.95_r, 
                       ymax = hi.95_r ), 
                  position = position_dodge( width = 0.5 ) ) + 
...
freq.predict.time2_plot <- freq.predict.time_plot + 
  annotate( "text", x = 1, y = 0.85, label = "A", size = 9 )
freq.predict.age2_plot <- freq.predict.age_plot + 
  annotate( "text", x = 1, y = 0.85, label = "B", size = 9 )

freq.plots <- gridExtra::grid.arrange( freq.predict.time2_plot, freq.predict.age2_plot )
freq.plots
ggsave( "freq_full.pred.pdf", plot = freq.plots, device = "pdf", path = "figures", width = 12, height = 15, units = "cm" )
