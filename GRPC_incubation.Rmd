...
freqData <- plyr::ddply( .data = fullBouts, 
                         ...
                         .fun = function( .df ){
                           ...
                           .start <- parse_date_time( x = paste( .start$measDate, .start$Time ), 
                                                      ...
                                                      tz = "America/Chicago" )
                           .start <- timeRounder( .start, .b, "ceiling" )
                           .end <- timeRounder( max( .df$EventStart ), .b, "floor" )                # and ends on the date after the last record
  ...
  left_join( y = dplyr::select( dayBouts,
                                id, Date, NestAge ),
             by = c( "id", "Date" ) ) %>% 
  left_join( y = dplyr::select( nests, 
                                NestID, contains( "Distance", ignore.case = TRUE ), Nest_Success, VOR_AVG ), 
             by = c( "id" = "NestID" ) )
...
yr.wilcox <- data.frame( X1 = intVars, X2 = "fYear", stringsAsFactors = FALSE )
...
Scorrs <- with( var.corrs, map2( .x = as.list( X1 ), 
                                 ...
                                 .f = function( xx, yy ) cor.test( x = fullBouts[ , xx ], 
                                                                   ...
                                                                   method = "spearman", 
                                                                   exact = FALSE ) ) )
Ytests <- with( yr.wilcox, map2( .x = as.list( X1 ), 
                                 .y = as.list( X2 ), 
...
yr.wilcox <- yr.wilcox %>% 
  mutate( WilcoxonW = sapply( Ytests, function( .htest ) .htest$statistic ), 
          WilcoxonP = sapply( Ytests, function( .htest ) .htest$p.value ) )

corrs.table <- var.corrs %>% 
  dplyr::select( -starts_with( "Pearson" ) ) %>% 
  mutate( X1 = sapply( X = X1, FUN = function( .nn ) names( intVars )[ intVars %in% .nn ] ), 
          X2 = sapply( X = X2, FUN = function( .nn ) names( intVars )[ intVars %in% .nn ] ), 
...
var.corrs <- data.frame( t( combn( x = intVars, m = 2 ) ), stringsAsFactors = FALSE )
yr.wilcox <- data.frame( X1 = intVars, X2 = "fYear", stringsAsFactors = FALSE )
tm.kruskal <- data.frame( X1 = intVars, X2 = "blockID", stringsAsFactors = FALSE )
...
Ttests <- with( tm.kruskal, map2( .x = as.list( X1 ), 
                                  .y = as.list( X2 ), 
...
tm.kruskal <- tm.kruskal %>% 
  mutate( KruskalX2 = sapply( Ttests, function( .htest ) .htest$statistic ), 
          KruskalP = sapply( Ttests, function( .htest ) .htest$p.value ) )
...
We created (generalized) linear mixed models ([G]LMMs) of off-bout duration and frequency using nest (hen) ID as a grouping factor in the random-effects structure. When considering covariates to include each model, we assessed correlations between continuous predictors using Spearman's rank correlation coefficient ($\rho$), and between continuous and categorical predictors using the Kruskal-Wallis rank sum test (when the categorical predictor had >2 levels) or the Mann-Whitney *U* test (when the categorical predictor had 2 levels). **<< discardment?? >>**

A Box-Cox power transformation of off-bout duration suggested that the variable was approximately log-normally distributed (maximum likelihood estimate for $\lambda$ = `r l_est.dur`). We therefore used a Gaussian error distribution and identity link function to model the natural-log-transformed duration. We considered the following covariates in models of log-transformed off-bout duration: year (as a categorical variable with two levels); nest age (days since the initiation of incubation); average VOR; wind speed; ordinal date; the signed difference between the ambient temperature (measured at the Ainsworth airport) and that of the nest just prior to off-bout initiation; and the distances from the nest to the nearest turbine, road, and water source. In addition, we considered the possibility for first-order interactions between nest age and ambient--nest temperature difference, between wind speed and the distance to the nearest wind turbine, and between ordinal date and the distance to the nearest water source. Thus, the global fixed-effects structure takes the form:

$$ \beta_0 + \beta_{Year} + \beta_{NestAge} + \beta_{NestAge:TempDiff} + \beta_{TempDiff} + \beta_{VOR} + \beta_{WindSpeed} + \beta_{WindSpeed:TurbineDistance} + \beta_{TurbineDistance} + \beta_{RoadsDistance} + \beta_{WaterDistance} + \beta_{WaterDistance:Date} + \beta_{Date}, $$

where $\beta_p$ represents the parameterized estimate for the effect of the variable $p$ on the response.

We modeled off-bout frequency as the log-likelihood that a hen initiated an off-bout during a particular 3-hour block of time during the recording period (*i.e.*, from the time of the first recorded off-bout until the nest was presumed to have hatched or failed). Because hens either did (probability = 1) or did not (probability = 0) initiate off-bouts during each block, this approach assumed a Bernoulli error distribution and used a logit link function to model the response. In addition to the variables listed above for the off-bout duration model (but using ambient temperature only rather than the temperature difference), we also considered precipitation and time of day as potential influences on the frequency of off-bout initation. For the latter model, we considered the possibility of first-order interactions between nest age and ambient temperature, between wind speed and the distance to the nearest wind turbine, between turbine distance and time block, between wind speed and time block, and between the distance to the nearest water source and ordinal date. Our global model of off-bout frequency had the form

$$ \beta_0 + \beta_{Year} + \beta_{NestAge} + \beta_{NestAge:Temp} + \beta_{Temp} + \beta_{VOR} + \beta_{Precipitation} + \beta_{WindSpeed} + \beta_{WindSpeed:TurbineDistance} + \beta_{TurbineDistance} + \beta{TurbineDistance:Time} + \beta_{Time} + \beta_{WindSpeed:Time} + \beta_{RoadsDistance} + \beta_{WaterDistance} + \beta_{WaterDistance:Date} + \beta_{Date}. $$

Before fitting the models, we mean-centered all continuous covariates and scaled them by one standard deviation to improve model convergence. We also used sum-to-zero contrasts on categorical predictors having >2 levels (*i.e.*, time block). Using a global (maximal) fixed-effects structure, we fit models with different random-effects strctures and selected among them using Akaike's Information Criterion with a second-order correction for small sample size (AIC$_c$; [@anderson2008]) following the procedure described by Zuur et al. [-@zuur2009] for selecting the most parsimonious mixed-effects model. The LMMs of off-bout duration were fit using restricted maximum likelihood (REML) to minimize bias in the random-effects estimates [@zuur2009]. We then selected a fixed-effects structure using the top-ranked random effects structure. Whereas Zuur et al. [-@zuur2009] recommend selecting the optimal fixed-effects structure by stepwise backwards selection from the global model, we compared this approach with one using AIC$_c$-based model selection for the off-bout duration analysis, and used only AIC$_c$-based selection to . In both cases, (and in both the duration and frequency analyses) models having different fixed effects were fitted and compared using maximum likelihood (ML) methods.
...
globalBoutsMod <- "numBouts ~ fYear + NestAge * AmbientTemp + Precipitation + VOR_AVG + ( WindSpeed + Turbine_Distance_m + blockID )^2 + Roads_Distance_m + Water_Distance_m * Jday"
...
#                                                       family = binomial( link = "logit" ) ) )
# 
# # CC <- convCheckR( freq.ranef_cs ) # 24 warnings and at least as many minutes.
# # UnID        : NONE!
# # LargeMaxGrad: year_int, all temp, wind.int, turb.int, all blck
# # LargeParGrad: year_int, all temp, wind.int, turb.int, all blck
...
# CC <- convCheckR( freq.ranef_cs.plus.bobyqa ) # 12 warnings
# UnID        : all year
# LargeMaxGrad: year, temp.int, wind.int, turb.int
# LargeParGrad: wind.int
# Singular    : all but temp, vor, wind, and turb
...
freq.fixef_forms <- list( int = numBouts ~ 1 + ( blockID | fID ), 
                          ...
                          tmp = numBouts ~ AmbientTemp + ( blockID | fID ), 
                          pcp = numBouts ~ Precipitation + ( blockID | fID ), 
                          vor = numBouts ~ VOR_AVG + ( blockID | fID ), 
                          wsp = numBouts ~ WindSpeed + ( blockID | fID ), 
                          ...
                          nag.tmp = numBouts ~ NestAge * AmbientTemp + ( blockID | fID ), 
                          nag_tmp = numBouts ~ NestAge + AmbientTemp + ( blockID | fID ), 
                          ws.td = numBouts ~ WindSpeed * Turbine_Distance_m + ( blockID | fID ), 
                          ws_td = numBouts ~ WindSpeed + Turbine_Distance_m + ( blockID | fID ), 
                          td.bk = numBouts ~ Turbine_Distance_m * blockID + ( blockID | fID ), 
                          td_bk = numBouts ~ Turbine_Distance_m + blockID + ( blockID | fID ), 
                          ws.bk = numBouts ~ WindSpeed * blockID + ( blockID | fID ), 
                          ws_bk = numBouts ~ WindSpeed + blockID + ( blockID | fID ), 
                          ws.td.bk = numBouts ~ ( WindSpeed + Turbine_Distance_m + blockID )^2 + ( blockID | fID ), 
                          ws_td_bk = numBouts ~ WindSpeed + Turbine_Distance_m + blockID + ( blockID | fID ), 
                          ws_td.bk = numBouts ~ WindSpeed + Turbine_Distance_m * blockID + ( blockID | fID ), 
                          ws.td_bk = numBouts ~ WindSpeed * Turbine_Distance_m + blockID + ( blockID | fID ), 
                          ws.bk_td = numBouts ~ WindSpeed * blockID + Turbine_Distance_m + ( blockID | fID ),
                          wtd.jdy = numBouts ~ Water_Distance_m * Jday + ( blockID | fID ), 
                          wtd_jdy = numBouts ~ Water_Distance_m + Jday + ( blockID | fID ), 
                          no.blk = numBouts ~ NestAge * AmbientTemp + WindSpeed * Turbine_Distance_m + blockID + ( blockID | fID ), 
                          whole = numBouts ~ NestAge * AmbientTemp + ( WindSpeed + Turbine_Distance_m + blockID )^2 + ( blockID | fID ), 
                          no.int = numBouts ~ fYear + NestAge + AmbientTemp + Precipitation + VOR_AVG + WindSpeed + Turbine_Distance_m + blockID + Roads_Distance_m + Water_Distance_m + Jday + ( blockID | fID ), 
                          global = numBouts ~ fYear + NestAge * AmbientTemp + Precipitation + VOR_AVG + ( WindSpeed + Turbine_Distance_m + blockID )^2 + Roads_Distance_m + Water_Distance_m * Jday + ( blockID | fID ) )
...
freq.test_aicc <- augment( freq.mod_aicc ) %>% 
  mutate( NestAge = ( NestAge * sd( freqData$NestAge, na.rm = TRUE ) ) + mean( freqData$NestAge, na.rm = TRUE ), 
          AmbientTemp = ( AmbientTemp * sd( freqData$AmbientTemp, na.rm = TRUE ) ) + mean( freqData$AmbientTemp, na.rm = TRUE ), 
          WindSpeed = ( WindSpeed * sd( freqData$WindSpeed, na.rm = TRUE ) ) + mean( freqData$WindSpeed, na.rm = TRUE ), 
          Turbine_Distance_m = ( Turbine_Distance_m * sd( freqData$Turbine_Distance_m, na.rm = TRUE ) ) + mean( freqData$Turbine_Distance_m, na.rm = TRUE ), 
...
rVat_freq.aicc <- ggplot( data = freq.test_aicc, 
                          aes( x = AmbientTemp, 
                               y = .qresid ) ) + 
  geom_hline( yintercept = 0, 
              linetype = "dashed" ) + 
...
rVws_freq.aicc <- ggplot( data = freq.test_aicc, 
                          aes( x = WindSpeed, 
                               y = .qresid ) ) + 
  geom_hline( yintercept = 0, 
              linetype = "dashed" ) + 
...
rVtd_freq.aicc <- ggplot( data = freq.test_aicc, 
                          aes( x = Turbine_Distance_m, 
                               y = .qresid ) ) + 
  geom_hline( yintercept = 0, 
              linetype = "dashed" ) + 
...
cowplot::plot_grid( rVna_freq.aicc, rVat_freq.aicc, rVws_freq.aicc, rVtd_freq.aicc, rVbk_freq.aicc, fVo_freq.aicc, nrow = 2, labels = "AUTO" )

```

```{r FREQpredictRwindSpeed}

nd <- expand.grid( NestAge = 0, 
                   AmbientTemp = 0, 
                   WindSpeed = ( seq( from = floor( min( freqData$WindSpeed, na.rm = TRUE ) ), 
                                      to = ceiling( max( freqData$WindSpeed, na.rm = TRUE ) ), 
                                      by = 0.5 ) - mean( freqData$WindSpeed, na.rm = TRUE ) ) / sd( freqData$WindSpeed, na.rm = TRUE ), 
                   Turbine_Distance_m = ( c( 500, 1000, 5000, 10000, 20000 ) - mean( freqData$Turbine_Distance_m, na.rm = TRUE ) ) / sd( freqData$Turbine_Distance_m, na.rm = TRUE ), 
...
if( !exists( "freq.predict_aicc.boot.ws" ) ){
  
  if( length( fl <- list.files( pattern = "freq.predict_aicc.boot.ws.RData", recursive = TRUE ) ) > 0 ){
    load( fl )
  } else {
    freq.predict_aicc.boot.ws <- bootMer( x = freq.mod_aicc, 
                                          FUN = boot.fitter, 
                                          nsim = 1000 )
    
    save( freq.predict_aicc.boot.ws, file = "data/freq.predict_aicc.boot.ws.RData" )
  }
}

freq.predict_aicc.boot.wsCI <- t( apply( X = freq.predict_aicc.boot.ws$t, 
                                         MARGIN = 2, 
                                         FUN = quantile, 
                                         probs = c( 0.025, 0.975 ) ) )
...
freq.predict_ws <- nd %>%
  mutate( .mu = predict( freq.mod_aicc, 
                         ...
                         re.form = ~0, 
                         type = "response" ), 
          lo.95 = freq.predict_aicc.boot.wsCI[ , 1 ],
          hi.95 = freq.predict_aicc.boot.wsCI[ , 2 ],
          NestAge_r = ( NestAge * sd( freqData$NestAge, na.rm = TRUE ) ) + mean( freqData$NestAge, na.rm = TRUE ), 
          AmbientTemp_r = ( AmbientTemp * sd( freqData$AmbientTemp, na.rm = TRUE ) ) + mean( freqData$AmbientTemp, na.rm = TRUE ), 
          WindSpeed_r = ( WindSpeed * sd( freqData$WindSpeed, na.rm = TRUE ) ) + mean( freqData$WindSpeed, na.rm = TRUE ), 
          Turbine_Distance_m_r = ( Turbine_Distance_m * sd( freqData$Turbine_Distance_m, na.rm = TRUE ) ) + mean( freqData$Turbine_Distance_m, na.rm = TRUE ), 
          lo.95_r = exp( lo.95 ), 
          hi.95_r = exp( hi.95 ) )

freq.predict_ws.plot <- ggplot( data = freq.predict_ws, 
                                aes( x = WindSpeed_r, 
                                     y = .mu, 
                                     linetype = factor( Turbine_Distance_m_r ) ) ) + 
  geom_line() + 
  ...
  labs( x = "Wind speed (mph)", 
        ...
        linetype = "Distance to nearest turbine (m)" ) + 
  facet_wrap( ~blockID )
...
nd <- expand.grid( NestAge = ( seq( from = floor( min( freqData$NestAge, na.rm = TRUE ) ), 
                                    to = ceiling( max( freqData$NestAge, na.rm = TRUE ) ), 
                                    by = 0.5 ) - mean( freqData$NestAge, na.rm = TRUE ) ) / sd( freqData$NestAge, na.rm = TRUE ), 
                   WindSpeed = 0, 
                   Jday = 0 )

if( !exists( "freq.predict_aicc.boot.na" ) ){
  
  if( length( fl <- list.files( pattern = "freq.predict_aicc.boot.na.RData", recursive = TRUE ) ) > 0 ){
    load( fl )
  } else {
    freq.predict_aicc.boot.na <- bootMer( x = freq.mod_aicc, 
                                          FUN = boot.fitter, 
                                          nsim = 1000 )
    
    save( freq.predict_aicc.boot.na, file = "data/freq.predict_aicc.boot.na.RData" )
  }
}

freq.predict_aicc.boot.naCI <- t( apply( X = freq.predict_aicc.boot.na$t, 
                                         MARGIN = 2, 
                                         FUN = quantile, 
                                         probs = c( 0.025, 0.975 ) ) )

freq.predict_na <- nd %>%
  mutate( .mu = predict( freq.mod_be, 
                         newdata = nd, 
                         re.form = ~0, 
                         type = "response" ), 
          lo.95 = freq.predict_aicc.boot.naCI[ , 1 ],
          hi.95 = freq.predict_aicc.boot.naCI[ , 2 ],
          NestAge_r = ( NestAge * sd( freqData$NestAge, na.rm = TRUE ) ) + mean( freqData$NestAge, na.rm = TRUE ), 
          WindSpeed_r = ( WindSpeed * sd( freqData$WindSpeed, na.rm = TRUE ) ) + mean( freqData$WindSpeed, na.rm = TRUE ), 
...
freq.predict_na.plot <- ggplot( data = freq.predict_na, 
                                aes( x = NestAge_r, 
                                     y = .mu ) ) + 
  geom_line() + 
  geom_rug( data = filter( freqData, numBouts > 0 ), 
...
We recorded `r nrow(bouts)` incubation off-bouts at `r length(unique(bouts$id))` nests. Eight nests (representing `r sum(is.na(bouts$NestAge))` off-bouts) could not be reliably aged, as they were not located until after clutch completion and failed prior to hatching. Because we considered nest age a potentially important factor for hens making behavioral decisions regarding incubation, data from those nests was not considered further.

The top-ranked random-effects structure for off-bout duration included both a random intercept and a random slope of the effect of 
