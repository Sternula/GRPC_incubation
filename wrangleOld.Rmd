...
nests <- inner_join( nestVeg, nests, by = c( "NestID" = "Nest" ) ) %>%
  ...
  dplyr::select( c( 1:5, 12, 54, 60:73 ) )
...
for( b in 1:nrow( bouts ) ){
  ...
  # Isolate the weather data from the same year pull out the two weather records closest in time to the start of the off-bout.
  boutWeather <- weather %>% 
    dplyr::select( DateTime, DryBulbCelsius ) %>% 
    filter( year( DateTime ) == year( boutStart ) ) %>%
    arrange( abs( DateTime - boutStart ) )
  boutWeather <- boutWeather[ 1:2, ]
  
  # Interpolate the ambient temperature based on the two nearest weather records. If the two nearest records are to one side (temporally) of the off-bout initiation time, the ambient temperature is taken to be the recorded temperature at the time nearest the start time (rule=2).
...
# Or not. Try the range .dbf (cropped and converted to a shapefile in ArcGIS on a separate machine by WJH) instead (also from USGS Gan Analysis Program).
GRPC_range <- readOGR( dsn = "data/distribution/GRPC_range/GPC_Range_NE", 
                       layer = "GPC_Range_NE" )

# Bring the GRPC_range shp into the desired projection and datum.
GRPC_range <- spTransform( GRPC_range, targProj )

# Prepare the range data for plotting with ggplot2.
GRPC_range@data$id <- rownames( GRPC_range@data )
GRPC_range.pts <- fortify( GRPC_range, region = "id" )
GRPC_range.df <- merge( GRPC_range.pts, GRPC_range@data, by = "id" )
...
GRPC_range.poly <- SpatialPolygons( Srl = list( GRPC_range = Polygons( srl = lapply( X = split( x = GRPC_range.df, f = GRPC_range.df$group ), 
                                                                                     FUN = function( .df ) Polygon( coords = .df[ , c( "long", "lat" ) ], 
                                                                                                                    hole = .df$hole ) ), 
                                                                       ID = "GRPC_range" ) ), 
                                    proj4string = CRS( targProj ) )
...
GRPC_range.dfc <- fortify( gIntersection( spgeom1 = GRPC_range.poly, spgeom2 = NE_poly ) )
...
globalMinsMod <- "Mins ~ fYear + NestAge * AmbientTemp + VOR_AVG + Turbine_within_1km + Roads_Distance_m + Water_Distance_m * Jday + Nest_Success"
globalMinsMod <- "Mins ~ NestAge * AmbientTemp + Jday"
...
dur.ranef <- list( int = lmer( as.formula( paste( globalMinsMod, " + ( 1 | fID )", sep = "" ) ), data = fullBouts ), 
                   ...
                   age.temp = lmer( as.formula( paste( globalMinsMod, " + ( NestAge * AmbientTemp - 1 | fID )", sep = "" ) ), data = fullBouts ), 
                   age.temp.int = lmer( as.formula( paste( globalMinsMod, " + ( NestAge * AmbientTemp | fID )", sep = "" ) ), data = fullBouts ), 
                   age.temp_int = lmer( as.formula( paste( globalMinsMod, " + ( 1 | fID ) + ( NestAge * AmbientTemp - 1 | fID )", sep = "" ) ), data = fullBouts ), 
                   temp = lmer( as.formula( paste( globalMinsMod, " + ( AmbientTemp - 1 | fID )", sep = "" ) ), data = fullBouts ), 
                   temp.int = lmer( as.formula( paste( globalMinsMod, " + ( AmbientTemp | fID )", sep = "" ) ), data = fullBouts ), 
                   temp_int = lmer( as.formula( paste( globalMinsMod, " + ( 1 | fID ) + ( AmbientTemp - 1 | fID )", sep = "" ) ), data = fullBouts ), 
                   ...
                   # nSuc = lmer( as.formula( paste( globalMinsMod, " + ( Nest_Success - 1 | fID )", sep = "" ) ), data = fullBouts ), 
                   # nSuc.int = lmer( as.formula( paste( globalMinsMod, " + ( Nest_Success | fID )", sep = "" ) ), data = fullBouts ), 
                   # nSuc_int = lmer( as.formula( paste( globalMinsMod, " + ( 1 | fID ) + ( Nest_Success - 1 | fID )", sep = "" ) ), data = fullBouts ) )


convCheckR( dur.ranef ) # age.temp, age.temp.int, age.temp_int, Jday, and Jday.int fail, but age.temp and Jday with OK relative gradients. age.temp, age.temp.int, and Jday_int singular

# Try scaling and centering continuous variables.

csfullBouts <- fullBouts %>% 
  mutate( NestAge = ( NestAge / sd( NestAge, na.rm = TRUE ) - mean( NestAge, na.rm = TRUE ) ), 
          AmbientTemp = ( AmbientTemp / sd( AmbientTemp, na.rm = TRUE ) ) - mean( AmbientTemp, na.rm = TRUE ), 
          Jday = ( Jday / sd( Jday ) ) - mean( Jday ) )
# contrasts( csfullBouts$fID ) <- contr.sum( length( levels( csfullBouts$fID ) ) )

dur.ranef_cs <- lapply( X = dur.ranef, 
                        FUN = function( .mm ) update( .mm, data = csfullBouts ) )

convCheckR( dur.ranef_cs ) # That actually seems to have made things worse. Do I also need to set the contrasts on the grouping variable?

cszfullBouts <- csfullBouts
contrasts( cszfullBouts$fID ) <- contr.sum( length( levels( cszfullBouts$fID ) ) )

dur.ranef_csz <- lapply( X = dur.ranef, 
                         FUN = function( .mm ) update( .mm, data = cszfullBouts ) )

convCheckR( dur.ranef_csz )
